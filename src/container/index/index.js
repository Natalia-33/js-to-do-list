export class Todo {
  ...
  static #handleAdd = () => {
    ...
    if (value.length > 1) this.#createTaskData(value)
    this.#input.value = ''
    this.#render()
    this.#saveData()
  }
  ...
  static #createTaskElem = (data) => {
    ...
    id.innerText = `${data.id}`.text.innerText = data.text
    ...
    if (data.done) {
      ...
      btnDo.classList.remove('task--button--do')
      btnDo.classList.add('task--button--done')
    }
